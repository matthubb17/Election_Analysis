...
* After helping Seth and Tom submit the election audit results, the election commission requested some additional data that they would like in order to complete the audit.
    ...
    - The county with the highest turnout
    	...
    	- Moving forward the election audit committee could use this script for upcoming elections. Since I properly commented out each line within the code by upating the script to pull in a new excel data file we could rerun the script with minimal tweaks and get updated results for a new election.
    	- In addition, if there are more votes that come in for this current election, we could update the current "election_results.csv" file and rerun the script to get updated results for the election along with identifying if the winner is the same. This will allow us to confirm we have the proper winner for this election.
